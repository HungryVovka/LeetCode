// 3. Longest Substring Without Repeating Characters
// 
// Given a string s, find the length of the longest substring without 
// duplicate characters.
// 
// Example 1:
// 
// Input: s = "abcabcbb"
// Output: 3
// Explanation: The answer is "abc", with the length of 3. 
// Note that "bca" and "cab" are also correct answers.
// 
// Example 2:
// 
// Input: s = "bbbbb"
// Output: 1
// Explanation: The answer is "b", with the length of 1.
// 
// Example 3:
// 
// Input: s = "pwwkew"
// Output: 3
// Explanation: The answer is "wke", with the length of 3.
// Notice that the answer must be a substring, "pwke" is a subsequence 
// and not a substring.
//  
// 
// Constraints:
// 
// 0 <= s.length <= 5 * 10^4
// s consists of English letters, digits, symbols and spaces.

#include <stddef.h>     // size_t
#include <string.h>     // strlen

int lengthOfLongestSubstring(char* s){
    int lastIndex[256];
    size_t stringLength;
    size_t leftIndex;
    size_t rightIndex;
    size_t bestLength;
    size_t candidateLength;
    size_t prevIndexPlusOne;
    unsigned char currentChar;
    int i;

    if (s == NULL) return 0;

    for (i = 0; i < 256; i++) lastIndex[i] = -1;

    stringLength = strlen(s);
    leftIndex = 0;
    bestLength = 0;

    for (rightIndex = 0; rightIndex < stringLength; rightIndex++){
        currentChar = (unsigned char)s[rightIndex];
        if (lastIndex[currentChar] >= 0){
            prevIndexPlusOne = (size_t)lastIndex[currentChar] + 1;
            if (prevIndexPlusOne > leftIndex) leftIndex = prevIndexPlusOne;
        }
        lastIndex[currentChar] = (int)rightIndex;
        candidateLength = rightIndex - leftIndex + 1;
        if (candidateLength > bestLength) bestLength = candidateLength;
    }
    return (int)bestLength;
}

// Tasks are the property of LeetCode (https://leetcode.com/) 
// and users of this resource.
// 
// All solution code in this repository 
// is the personal property of Vladimir Rukavishnikov
// (vladimirrukavishnikovmail@gmail.com).
...
# The optimal path with at most 1 stop from city 0 to 3 is marked 
# in red and has cost 100 + 600 = 700.
...
# Explanation:
# The graph is shown above.
# The optimal path with no stops from city 0 to 2 is marked in red 
# and has cost 500.
# 
# Constraints:
# 
...
class Solution:
    def dijkstra(self,\
    graph: dict, src: int, dst: int, k: int, cache: dict) -> int:
        pq = [(0, src, k + 1)]
        while pq:
            ...
            if stops > 0:
                ...
                for neighbor, price in neighbors:
                    new_cost = cost + price
                    if (neighbor, stops - 1) not in cache \
                    or new_cost < cache[(neighbor, stops - 1)]:
                        cache[(neighbor, stops - 1)] = new_cost
    ...
    n: int, flights: List[List[int]], src: int, dst: int, k: int) -> int:
        ...
        cache = {}
        return self.dijkstra(graph, src, dst, k, cache)

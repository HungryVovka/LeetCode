...
// Output: [2,7,4]
// Explanation: We return the node with value 2, colored in yellow in 
// the diagram.
...
static struct Info dfs(struct TreeNode *root){
    struct Info answer;
    struct Info leftInfo;
    ...
    if (leftInfo.depth > rightInfo.depth){
        answer.depth = leftInfo.depth + 1;
        answer.node = leftInfo.node;
        return answer;
    }
    if (rightInfo.depth > leftInfo.depth){
        answer.depth = rightInfo.depth + 1;
        answer.node = rightInfo.node;
        return answer;
    }
    answer.depth = leftInfo.depth + 1;
    answer.node = root;

...
class Solution:
    def mostBooked(self, n: int, meetings: List[List[int]]) -> int:
        ...
        available_room_ids = [i for i in range(n)]
        heapq.heapify(available_room_ids)
        for start, end in meetings:
            while occupied and occupied[0][0] <= start:
                heapq.heappush(available_room_ids, heapq.heappop(occupied)[1])
            if available_room_ids:
                room_id = heapq.heappop(available_room_ids)
                count[room_id] += 1
                heapq.heappush(occupied, (end, room_id))
            else:
                new_start, room_id = heapq.heappop(occupied)
                count[room_id] += 1

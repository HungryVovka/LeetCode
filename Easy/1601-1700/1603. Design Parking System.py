...
# represented by 1, 2, and 3 respectively. A car can only park in a parking space of its carType. If 
...
class ParkingSystem:
    ...
    def addCar(self, carType: int) -> bool:
        match carType:
            case 1 if self.count[0] > 0:
                self.count[0] -= 1
                return True
            case 2 if self.count[1] > 0:
                self.count[1] -= 1
                return True
            case 3 if self.count[2] > 0:
                self.count[2] -= 1
                return True
            case _:
                return False
